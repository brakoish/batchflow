  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div className="space-y-2.5">
        <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Existing</h2>
        {recipes.length === 0 ? (
              ...
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-1 shrink-0 ml-2">
                  ...
                  <button onClick={() => handleDelete(recipe.id)} disabled={deleting === recipe.id}
                    className="p-1.5 rounded-lg text-muted-foreground hover:text-red-500 hover:bg-muted transition-colors disabled:opacity-40">
                    <TrashIcon className="w-3.5 h-3.5" />
                  </button>
                  <span className="text-[10px] text-muted-foreground/60 ml-1 tabular-nums">{recipe._count.batches}</span>
                </div>
              </div>
